...
class Avatar:
    ...
    def inference(self, 
        ...
        if out_vid_name is not None and args.skip_save_images is False: 
            # Save the video file in the FFmpeg filesystem
            temp_output = "/tmp/temp_output.mp4"
            cmd_img2video = f"ffmpeg -y -v warning -r {fps} -f image2 -i {self.avatar_path}/tmp/%08d.png -vcodec libx264 -vf format=rgb24,scale=out_color_matrix=bt709,format=yuv420p -crf 18 {temp_output}"
            print(cmd_img2video)
            try:
                result = subprocess.run(cmd_img2video, shell=True, check=True, capture_output=True, text=True)
                print("FFmpeg output:", result.stdout)
            except subprocess.CalledProcessError as e:
                print("FFmpeg error:", e.stderr)
                return  # Exit the function if FFmpeg fails

            # Combine audio
            final_output = "/tmp/final_output.mp4"
            cmd_combine_audio = f"ffmpeg -y -v warning -i {audio_path} -i {temp_output} {final_output}"
            print(cmd_combine_audio)
            try:
                result = subprocess.run(cmd_combine_audio, shell=True, check=True, capture_output=True, text=True)
                print("FFmpeg output:", result.stdout)
            except subprocess.CalledProcessError as e:
                print("FFmpeg error:", e.stderr)
                return  # Exit the function if FFmpeg fails
            ...
            try:
                ...
                s3_key = f"videos/{out_vid_name}.mp4"
                
                s3_client.upload_file(final_output, bucket_name, s3_key)
                print(f"Video uploaded successfully to S3: s3://{bucket_name}/{s3_key}")
