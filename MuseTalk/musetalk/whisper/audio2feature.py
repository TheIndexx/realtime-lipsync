...
class Audio2Feature():
    def __init__(self, 
                 ...
                 model_path="/vol/whisper/tiny.pt"):
    ...
    def feature2chunks(self, feature_array, fps, audio_feat_length=[2, 2]):
        """
        Convert a feature array into chunks based on the video frame rate.

        This function takes a feature array (typically from audio processing) and splits it into
        chunks that correspond to video frames. It uses the `get_sliced_feature` method to extract
        relevant features for each video frame.

        Args:
            feature_array (numpy.ndarray): The input feature array to be chunked.
            fps (float): Frames per second of the video.
            audio_feat_length (list): A list of two integers specifying the number of frames
                                      to consider before and after the current frame. 
                                      Default is [2, 2].

        Returns:
            list: A list of numpy arrays, where each array represents the audio features
                  corresponding to a video frame.

        Note:
            - The function assumes that the audio features are sampled at 50 Hz.
            - It uses a multiplier (50/fps) to map video frame indices to audio feature indices.
            - The process continues until all features in the input array have been processed.
        """
        whisper_chunks = []
    ...
    def audioData2feat(self,audio_data):
        # get the sample rate of the audio
        result = torch.from_numpy(audio_data).float()
        embed_list = []
        for emb in result['segments']:
            encoder_embeddings = emb['encoder_embeddings']
            encoder_embeddings = encoder_embeddings.transpose(0, 2, 1, 3)
